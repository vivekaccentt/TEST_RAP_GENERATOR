...
CLASS lsc_ZDMO_R_RAPG_PROJECTTP IMPLEMENTATION.
  ...
  METHOD save_modified.
    ...
    DATA bgpf_del_operation TYPE REF TO zdmo_cl_rap_generator_del_bgpf .
    ...
    LOOP AT update-project INTO DATA(update_rapgeneratorbo)
      ...
      TRY.
          IF update_rapgeneratorbo-BoIsDeleted = abap_true.
            bgpf_del_operation = NEW zdmo_cl_rap_generator_del_bgpf(
          ...
          ELSE.
* comment out on prem prior to 2023
...
**      lo_parallel->run_inst(
**        EXPORTING
**          p_in_tab  = VALUE #( ( bgpf_del_operation ) )
**        IMPORTING
**          p_out_tab = DATA(lt_out)
**      ).
*
*          background_process = cl_bgmc_process_factory=>get_default(  )->create(  ).
*          background_process->set_operation_tx_uncontrolled( bgpf_del_operation ).
*          background_process->set_name( CONV #( |Delete { update_rapgeneratorbo-BoName }| ) ).
*          background_process->save_for_execution(  ).
*
*        CATCH cx_bgmc INTO DATA(del_exception).
*          "handle exception
*          latest_t100_exception = cl_message_helper=>get_latest_t100_exception( exception ).
*          IF latest_t100_exception IS NOT INITIAL.
*            DATA(del_exception_text) = latest_t100_exception->if_message~get_text(  ).
*          ELSE.
*            del_exception_text = exception->get_text(  ).
*          ENDIF.
*
*      ENDTRY.
*
